...
router.post("/register", upload.single("avatar"), async (req, res) => {
  ...
  else if (typeof newUser == "string") return res.status(400).send(newUser);
...
router.put("/changePassword", async (req, res) => {
  ...
  else if (typeof user == "string") return res.status(400).send(user);
...
router.get("/getInvoicesOnUsers", async (req, res) => {
  let users = await getInvoicesOnUsers(req);
